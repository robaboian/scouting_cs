...
@st.cache_data
def load_data():
    return pd.read_csv('data/defensores centrales.csv')

# App principal
def main():
    st.set_page_config(page_title="Defensores centrales", layout="wide")
    st.subheader("Defensores centrales")

    df = load_data()

    df['Perfil ofensivo'] = (df['Gol y Finalización']*0.1 + df['Asistencias y creación de chances']*0.1 +
                             df['1v1 en ataque']*0.1 + df['Progresion de pelota']*0.25 +
                             df['Juego asociado']*0.25 + df['Juego aéreo']*0.2).astype(int)

    df['Perfil defensivo'] = (df['1v1 en defensa']*0.4 + df['Defensa']*0.4 +
