...
@st.cache_data
def load_data():
    return pd.read_csv('data/centrodelanteros.csv')

# App principal
def main():
    st.set_page_config(page_title="Centrodelanteros")
    st.subheader("Centrodelanteros")

    df = load_data()


    df['Perfil ofensivo'] = (df['Gol y Finalización']*0.35 + 
                         df['Asistencias y creación de chances']*0.15 +
                         df['1v1 en ataque']*0.1 + 
                         df['Centros']*0.05 +
                         df['Juego asociado']*0.2 + 
                         df['Juego aéreo']*0.15).astype(int)

    df['Perfil defensivo'] = (df['Defensa']*0.7 + 
                          df['Juego aéreo']*0.3).astype(int)
