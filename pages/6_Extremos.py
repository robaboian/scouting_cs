...
@st.cache_data
def load_data():
    return pd.read_csv('data/extremos.csv')

# App principal
def main():
    st.set_page_config(page_title="Extremos")
    st.subheader("Extremos")

    df = load_data()

    df['Perfil ofensivo'] = (df['Gol y Finalización']*0.2 + 
                         df['Asistencias y creación de chances']*0.25 +
                         df['1v1 en ataque']*0.25 + 
                         df['Centros']*0.2 + 
                         df['Juego asociado']*0.1).astype(int)

    df['Perfil defensivo'] = (df['Defensa']*0.9 +
    ...
    opcion_extremo = st.selectbox("Puesto:",
    ["Sin asignar", "Extremo por derecha", "Extremo por izquierda"])

    if opcion_extremo == "Extremo por derecha":
        df = df[df['Position'].str.contains('R', na=False)]
    elif opcion_extremo == "Extremo por izquierda":
