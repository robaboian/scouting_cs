...
@st.cache_data
def load_data():
    return pd.read_csv('data/volantes defensivos.csv')

# App principal
def main():
    st.set_page_config(page_title="Volantes defensivos", layout="wide")
    st.subheader("Volantes defensivos")

    df = load_data()

    df['Perfil ofensivo'] = (df['Gol y Finalización']*0.05 + df['Asistencias y creación de chances']*0.2 +
                             df['1v1 en ataque']*0.1 + df['Progresion de pelota']*0.25 +
                             df['Juego asociado']*0.3 + df['Juego aéreo']*0.1).astype(int)
