...
@st.cache_data
def load_data():
    return pd.read_csv('data/laterales.csv')
...
def main():
    ...
    df['Perfil ofensivo'] = (df['Gol y Finalización']*0.1 + df['Asistencias y creación de chances']*0.25 +
                            df['1v1 en ataque']*0.2 + df['Centros']*0.25 + df['Juego asociado']*0.2).astype(int)
    

    df['Perfil defensivo'] = (df['1v1 en defensa']*0.45 + df['Defensa']*0.45 +
                              df['Juego aéreo']*0.1).astype(int)
    ...
    opcion_lateral = st.selectbox("Puesto:",
...
# Scatter general
    ax2.scatter(df['Perfil ofensivo'], df['Perfil defensivo'],
            color='lightblue', alpha=0.1,
            s=df['Juego asociado'] * 1.5)

# Jugador/es destacado/s
    if data_jugador_2 is not None:
        ax2.scatter(data_jugador_1['Perfil ofensivo'], data_jugador_1['Perfil defensivo'],
                color='white', label=texto_jugador_1,
                s=data_jugador_1['Juego asociado'] * 3, alpha=1, edgecolors="black")

        ax2.scatter(data_jugador_2['Perfil ofensivo'], data_jugador_2['Perfil defensivo'],
                color='yellow', label=texto_jugador_2,
                s=data_jugador_2['Juego asociado'] * 3, alpha=1, edgecolors="black")
    else:
        ax2.scatter(data_jugador_1['Perfil ofensivo'], data_jugador_1['Perfil defensivo'],
                color='white', label=texto_jugador_1,
                s=data_jugador_1['Juego asociado'] * 3, alpha=1, edgecolors="black")
