...
@st.cache_data
def load_data():
    return pd.read_csv('data/volantes mixtos.csv')

# App principal
def main():
    st.set_page_config(page_title="Volantes mixtos", layout="wide")
    st.subheader("Volantes mixtos")

    df = load_data()


    df['Perfil ofensivo'] = (df['Gol y Finalización']*0.15 + df['Asistencias y creación de chances']*0.15 +
                             df['1v1 en ataque']*0.2 + df['Centros']*0.15 +
                             df['Juego asociado']*0.3 + df['Juego aéreo']*0.05).astype(int)
